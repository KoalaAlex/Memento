...
public class MadDragStopDraggable : MadDraggable {
    ...
    public bool limitSwipeToSinglePage = false;
    ...
    protected override void Update() {
        ...
        if (!IsTouchingSingle()) {
            ...
            if (IsTouchingJustStarted()) {
                ...
                var delta = touchPos - lastPosition;
                var touchSpeedDrag = ComputeDrag();
                delta *= (1 - touchSpeedDrag);
                
                delta = ApplyScreenAspect(delta);
    ...
    private float ComputeDrag() {
        var closestDragStop = ClosestDragStopIndex();
        var dragStopPos = dragStops[closestDragStop];
        if (closestDragStop == 0 && cameraPos.x < dragStopPos.x || closestDragStop == dragStopCount - 1 && cameraPos.x > dragStopPos.x) {
            float delta = Mathf.Abs(cameraPos.x - dragStopPos.x);
            float f = delta / Screen.width * 2;
            return f;
        }
    ...
    int IntendedDragStopIndex() {
        ...
        var force = GetInertiaForce();
        float thredshold = avarageDistance * swipeVirtualDistanceModifier;

        int newIndex = index;
        if (force > thredshold) {
            newIndex = Mathf.Clamp(index + IndexChange(force, thredshold), 0, dragStops.Count);
        } else if (force < -thredshold) {
            newIndex = Mathf.Clamp(index - IndexChange(force, thredshold), 0, dragStops.Count);
        }

        newIndex = Mathf.Clamp(newIndex, 0, dragStops.Count - 1);
        if (limitSwipeToSinglePage) {
            newIndex = Mathf.Clamp(newIndex, index - 1, index + 1);
        }

        if (newIndex == index) {
            var currentDragStop = dragStops[index];
            if (Vector2.Distance(currentDragStop, cameraPos) > avarageDistance * switchAfterDistance) {
                newIndex = ClosestDragStopIndex(index);

                if (Vector2.Distance(dragStops[newIndex], cameraPos) > avarageDistance) {
                    // Cancel move if distance is greater than distance between pages.
                    // This usually means that we would go to a different direction
                    newIndex = index;
                }
            }
    ...
    private float GetInertiaForce() {
        switch (direction) {
            case Direction.Horizontal:
                return directionInvert ? -inertiaForce.x : inertiaForce.x;
            case Direction.Vertical:
                return directionInvert ? inertiaForce.y : -inertiaForce.y;
            default:
